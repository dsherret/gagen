...
`action()` generates the `action.yml` of a
[composite action](https://docs.github.com/en/actions/sharing-automations/creating-actions/creating-a-composite-action)
from the same steps API, so inputs are typed, step ordering and condition
propagation work the same as in a job, and `uses:` references get pinned.

```ts
// .github/workflows/action.ts
import { action, defineInputs, step } from "gagen";

const inputs = defineInputs({
  version: { description: "Version to install", default: "latest" },
  cache: { description: "Cache downloads", default: "false" },
});

const install = step({
  name: "Install",
  // inputs.version is an ExpressionValue("inputs.version")
...
const restore = step({
  ...
  with: { path: "~/.cache/tool", key: "tool-${{ runner.os }}" },
...
action({
  ...
  // composite actions have no `defaults` block, so this is applied to every
  // `run` step that doesn't set its own shell (GitHub requires one)
  defaults: { run: { shell: "bash" } },
  steps: [install, restore],
  branding: { icon: "download", color: "blue" },
}).writeOrLint({
  filePath: new URL("../../action.yml", import.meta.url),
  header:
    "# GENERATED BY ./.github/workflows/action.ts -- DO NOT DIRECTLY EDIT",
});
```
