...
// --- null / undefined step support ---

Deno.test("null steps in job steps array are ignored", () => {
  setup();
  const checkout = step({ uses: "actions/checkout@v6" });
  const wf = createWorkflow({
    name: "test",
...
Deno.test("null steps in composite step are filtered out", () => {
  setup();
  const a = step({ name: "A", run: "a" });
  const b = step({ name: "B", run: "b" });
  const composite = step(a, null, b);
  const wf = createWorkflow({
    name: "test",
    on: {},
    jobs: [{
...
Deno.test("null deps in dependsOn are ignored", () => {
  setup();
  const checkout = step({ uses: "actions/checkout@v6" });
  const build = step.dependsOn(checkout, null, undefined)({
    name: "Build",
    run: "cargo build",
  });
  const wf = createWorkflow({
...
Deno.test("null deps in comesAfter are ignored", () => {
  setup();
  const a = step({ name: "A", run: "a" });
  const b = step({ name: "B", run: "b" });
  const c = b.comesAfter(a, null);
  const wf = createWorkflow({
    name: "test",
    on: {},
    jobs: [{
...
Deno.test("conditional step via ternary produces correct output", () => {
  setup();
  const useLint = false;
  const checkout = step({ uses: "actions/checkout@v6" });
  const lint = useLint ? step({ name: "Lint", run: "npm run lint" }) : null;
  const test = step.dependsOn(checkout, lint)({
    name: "Test",
    run: "npm test",
  });
  const wf = createWorkflow({
